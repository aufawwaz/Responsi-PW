...
<div class="bg-white min-h-screen">
    <div class="container mx-auto px-4 py-32 max-w-2xl">
        <h2 class="text-2xl font-bold mb-6 text-gray-800 flex items-center gap-2">
            <span class="text-2xl">✏️</span> Edit Judul Komik
        </h2>
        ...
        <form action="{{ route('komik.updateName', $komik->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label class="block mb-1 font-medium text-gray-700">Judul Baru</label>
                <input type="text" name="title" value="{{ old('title', $komik->title) }}"
